...
const unsigned MAX_TURNS = 127; // Updated for new Maxn. before: 100. now: 125
...
int computeSolution(const std::vector<unsigned>& initialGold,
  ...
  while (!pq.empty()) {
    ...
    // First, try to stay at same place
    {
      ...
      auto newIt = distance.find(newDistanceState); // this changed. I checked same state before.
      if (newIt == distance.end() || newIt->second < newGold) {
        if (newIt == distance.end()) distance[newDistanceState] = newGold;
        else newIt->second = newGold;
